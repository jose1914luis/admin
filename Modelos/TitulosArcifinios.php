<?php
/*
	Clase encargada de administrar la información relacionada a titulos y sus correspondientes puntos arcifinios
	en el CMQ
*/

	class TitulosArcifiniosTMP {		
		var $conn;
		...
		function selectAll() {
			...
			$queryStr =  'SELECT * FROM TIT_ARCIFINIOS_TMP ORDER BY id_solicitud';			
		...
		function insertAll($sol) {
			...
			if(!isset($sol["planchaIgac"])) $sol["planchaIgac"] = "";
			
			$queryStr =  " insert into TIT_ARCIFINIOS_TMP (
				ID_TITULO,		
				PLANCHA_IGAC,		
				SISTEMA_ORIGEN,		
				DESCRIPCION_PA
		...
		function deleteArcifinios($idTitulo) {
			if (!$this->conn) {
				echo "Error de Conexión con enlaces superiores.\n";
				return 0;
			}
			
			$queryStr =  "delete from TIT_ARCIFINIOS_TMP where id_titulo=$1";			
			$params = array($idTitulo);
			$result = pg_query_params($this->conn, $queryStr, $params);
			if(pg_last_error($this->conn))
			echo "<table bgcolor='yellow' border = 0><tr><td>Error al eliminar punto arcifinio del titulo</td></tr><tr><td>".pg_last_error($this->conn)."</td></tr></table>";			
		}
