...
*/
	...
	class SolicitudesCgBog {	
		...
		function asignarSistemaOrigen() {
			$idSolicitud = $this->idSolicitud;
			$origenPoly = $this->origen;  // valor del srid gauss Bogotá
			$queryStr =  "select plancha_igac, sistema_origen from sol_arcifinios_tmp where id_solicitud=$1 limit 1";			
			...
			if($ERROR) 
				echo "<table bgcolor='red' border = 0><tr><td>Error al consultar sistema de origen y plancha igac. $ERROR</td></tr></table>";
			else {
				$validacion = pg_fetch_all($result);	
				if(@$validacion[0]["plancha_igac"] != "")
					$origenPoly = $this->planchasIgac[trim($validacion[0]["plancha_igac"])];
				else if (@$validacion[0]["sistema_origen"] != "");
					$origenPoly = $this->sistemasOrigen[trim($validacion[0]["sistema_origen"])];
			}
			return $origenPoly;	
		}
		...
		function selectAll() {
			$queryStr =  'SELECT * FROM SOLICITUDES_CG_BOG ORDER BY ID_SOLICITUD';			
		...
		function existePlaca($id_solicitud) {
			$queryStr =  'SELECT count(1) as existe FROM SOLICITUDES_CG_BOG WHERE id_solicitud=$1 limit 1';			
			$params = array($id_solicitud);	
			
			$result = pg_query_params($this->conn, $queryStr, $params);
			if (!$result) {
			  echo "Error al consultar existencia de la solicitud.\n";
			  return 0;
			}						
			$lista = pg_fetch_all($result);			
			pg_free_result($result);
		...
		function insertAll() {
			...
			$this->origen = $this->asignarSistemaOrigen();
			...
			if (!$hayErroresGeometricos) {
				...
				$this->deleteTo($this->idSolicitud);		
				$this->deleteWGS84To($this->idSolicitud);					
			
				if($origenBD==$this->origen) {
					$queryStr		=  " insert into SOLICITUDES_CG_BOG (id_solicitud, the_geom, area_cmc, id_usuario) values ($1, ST_GeomFromText($2, 21897), $3, $4)";			
					$queryStrWGS84 	=  " insert into SOLICITUDES_CG (id_solicitud, the_geom) values ($1, ST_Transform(ST_Transform(ST_GeomFromText($2, 21897),".$this->datumBogota."),".$this->wgs84."))";			
				}	else	{
					$queryStr 		=  " insert into SOLICITUDES_CG_BOG (id_solicitud, the_geom, area_cmc, id_usuario) values (	$1, ST_Transform(ST_GeomFromText($2, ".$this->origen."), $origenBD), $3, $4)";			
					$queryStrWGS84 	=  " insert into SOLICITUDES_CG (id_solicitud, the_geom) values (	$1, ST_Transform(ST_Transform(ST_Transform(ST_GeomFromText($2, ".$this->origen."), $origenBD), ".$this->datumBogota."),".$this->wgs84."))";			
				}	
				$params = array($this->idSolicitud, $coords, $this->areaPoliCMC, $this->idUsuario);
				$params2 = array($this->idSolicitud, $coords);
		
				$result = pg_query_params($this->conn, $queryStr, $params);
				
				$ERROR = pg_last_error($this->conn);
				if($ERROR) 
					echo "<table bgcolor='red' border = 0><tr><td>Error al Insertar Coordenadas Planas: $ERROR</td></tr></table>";	
				else  
					$this->updateAreaTo($this->idSolicitud);

				$result = pg_query_params($this->conn, $queryStrWGS84, $params2);
				$ERROR = pg_last_error($this->conn);
				if($ERROR)
					echo "<table bgcolor='red' border = 0><tr><td>Error al Insertar Coordenadas Geográficas WGS84: $ERROR</td></tr></table>";	
				else {
					// calculo del porcentaje de precisión
					$queryStr		=  "select error_captura from SOLICITUDES_CG_BOG where id_solicitud=$1 limit 1"; 
					$result = pg_query_params($this->conn, $queryStr, array($this->idSolicitud));
					$valor_error = pg_fetch_all($result);
					
					if($valor_error[0]["error_captura"] > 0.005) 
						echo "<table bgcolor='red' border = 0><tr><td><font color='white'><b>El Porcentaje de error entre el area del poligono y la capturada es mayor al permitido (0,005%): {$valor_error[0]["error_captura"]}</b></font></td></tr></table>";
				}
			} else 
		...
		function deleteTo($idSol) {	
			$queryStr =  "delete from SOLICITUDES_CG_BOG where id_solicitud=$1";			
			$params = array($idSol);
		...
		function deleteWGS84To($idSol) {	
			$queryStr =  "delete from SOLICITUDES_CG where id_solicitud=$1";			
			$params = array($idSol);
		...
		function updateAreaTo($idSol) {	
			$queryStr =  "
				update SOLICITUDES_CG_BOG 
					set area=ST_Area(the_geom), 
					perimetro=ST_Perimeter(the_geom), 
					error_captura= (100*abs(area_cmc-ST_Area(the_geom))/area_cmc)::numeric
		...
		function getCentroideWGS84($idSolicitud) {
			$queryStr =  "select ST_AsText(ST_centroid(the_geom)) as centroide from solicitudes_cg where id_solicitud=$idSolicitud";			
			
			$result = pg_query($this->conn, $queryStr);
			if (!$result) {
			  echo "Error al obtener centroide de la solicitud.\n";
			  return 0;
			}						
			$lista = pg_fetch_all($result);			
			pg_free_result($result);
		...
		function getArea($idSolicitud) {
			$queryStr =  "select (area/10000) as area from solicitudes_cg_bog where id_solicitud=$idSolicitud";			
