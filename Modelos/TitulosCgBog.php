...
*/
	...
	class TitulosCgBog {	
		...
		function __construct($id_titulo, $coordenadas="", $area_poli_cmc=0, $idUsuario=0) {
			$this->Coordenadas = $coordenadas;	
			$this->idTitulo = $id_titulo;
		...
		function asignarSistemaOrigen() {
			...
			$queryStr =  "select plancha_igac, sistema_origen from tit_arcifinios_tmp where id_titulo=$1 limit 1";			
		...
		function selectAll() {
			$queryStr =  'SELECT * FROM TITULOS_CG_BOG ORDER BY id_titulo';			
		...
		function existePlaca($id_titulo) {
			$queryStr =  'SELECT count(1) as existe FROM TITULOS_CG_BOG WHERE id_titulo=$1 limit 1';			
			$params = array($id_titulo);	
			
			$result = pg_query_params($this->conn, $queryStr, $params);
			if (!$result) {
			  echo "Error al consultar existencia del Titulo.\n";
			  return 0;
			}						
			$lista = pg_fetch_all($result);			
			pg_free_result($result);
		...
		function insertAll() {
			...
			if (!$hayErroresGeometricos) {
				...
				$this->deleteTo($this->idTitulo);		
				$this->deleteWGS84To($this->idTitulo);					
			
				if($origenBD==$this->origen) {
					$queryStr		=  " insert into TITULOS_CG_BOG (id_titulo, the_geom, area_cmc, id_usuario) values ($1, ST_GeomFromText($2, 21897), $3, $4)";			
					$queryStrWGS84 	=  " insert into TITULOS_CG (id_titulo, the_geom) values ($1, ST_Transform(ST_Transform(ST_GeomFromText($2, 21897),".$this->datumBogota."),".$this->wgs84."))";			
				}	else	{
					$queryStr 		=  " insert into TITULOS_CG_BOG (id_titulo, the_geom, area_cmc, id_usuario) values (	$1, ST_Transform(ST_GeomFromText($2, ".$this->origen."), $origenBD), $3, $4)";			
					$queryStrWGS84 	=  " insert into TITULOS_CG (id_titulo, the_geom) values (	$1, ST_Transform(ST_Transform(ST_Transform(ST_GeomFromText($2, ".$this->origen."), $origenBD), ".$this->datumBogota."),".$this->wgs84."))";			
				}	
				$params = array($this->idTitulo, $coords, $this->areaPoliCMC, $this->idUsuario);
				$params2 = array($this->idTitulo, $coords);
				...
				if($ERROR) 
					...
					$this->updateAreaTo($this->idTitulo);
				...
				if($ERROR)
					...
					$queryStr		=  "select error_captura from TITULOS_CG_BOG where id_titulo=$1 limit 1"; 
					$result = pg_query_params($this->conn, $queryStr, array($this->idTitulo));
		...
		function deleteTo($idSol) {	
			$queryStr =  "delete from TITULOS_CG_BOG where id_titulo=$1";			
			$params = array($idSol);
		...
		function deleteWGS84To($idSol) {	
			$queryStr =  "delete from TITULOS_CG where id_titulo=$1";			
			$params = array($idSol);
		...
		function getCentroideWGS84($idTitulo) {
			$queryStr =  "select ST_AsText(ST_centroid(the_geom)) as centroide from TITULOS_CG where id_titulo=$idTitulo";			
			
			$result = pg_query($this->conn, $queryStr);
			if (!$result) {
			  echo "Error al obtener centroide del titulo.\n";
			  return 0;
			}						
			$lista = pg_fetch_all($result);			
			pg_free_result($result);
		...
		function getArea($idTitulo) {
			$queryStr =  "select (area/10000) as area from TITULOS_CG_BOG where id_titulo=$idTitulo";			
